<template lang="pug">
  div
    ...
    v-divider
    v-text-field(filled,v-model='itemsFilter',placeholder="filtrar navegação...")
    //-> Custom Navigation
    ...
    v-list.py-2(v-else-if='currentMode === `browse`', dense, :class='color', :dark='dark')
      ...
      template(v-for='item of currentItems')
        v-list-item(v-if='item.isFolder', :key='`childfolder-` + item.id', @click='PwtCheckAndLoadItem(item)')
          v-list-item-avatar(size='24')
            v-icon mdi-folder
          v-list-item-title {{ item.title }}
...
export default {
  ...
  methods: {
	...
	,async PwtCheckAndLoadItem(item){
		return PwtSidebarItemClik({
			 vuec:this
			,item
		});
	}
	...
	,async InvokeTreeQl(pathOrParent,locale,ancestor = false){
	
		var query = gql`
		  query ($parent: Int, $locale: String!, $path:String, $anc:Boolean) {
		  pages {
			tree(parent: $parent, path:$path,mode: ALL, locale: $locale, includeAncestors: $anc) {
			id
			path
			title
			isFolder
	...
	,async fetchBrowseItems (item) {
      ...
      const resp = await this.InvokeTreeQl(item.id,this.locale);
	...
	,async loadFromCurrentPath(currentParentId = null) {
	
	  return PwtLoadNavCurrentPath({
			vuec: this
			,lafunc: {
				 get	: _.get
				,find	: _.find
		...
		/*
      this.$store.commit(`loadingStart`, 'browse-load')
	  const resp = await this.InvokeTreeQl(this.path,this.locale);
	...
	,watch:{
		itemsFilter:async function(n,o){
		
			return PwtSideBarFilter({
				vuec: this
				,oldVal:o 
				,newVal:n
			})
